...
// CSS de react-grid-layout (placement, resize handle, transitions)
...
/** Forme persistée : un tableau de widgets + leur layout par breakpoint. */
...
function defaultState(): CockpitState {
  // Disposition initiale soignée — démontre toutes les capacités.
  ...
  // Variantes responsives simples : on empile.
...
function loadState(): CockpitState {
  try {
    ...
    // Filtre des widgets dont le kind n'existe plus dans le registre
...
export default function Cockpit() {
  ...
  const removeWidget = useCallback((id: string) => {
    setState((s) => {
      ...
      const layouts: Layouts = Object.fromEntries(
        Object.entries(s.layouts).map(([bp, items]) => [bp, items.filter((it) => it.i !== id)]),
      ) as Layouts;
  ...
  const addWidget = useCallback((kind: WidgetKind) => {
    ...
    setState((s) => {
      // calcule un y libre = max y existant + 1
      const lg = s.layouts.lg ?? [];
      const nextY = lg.reduce((acc, it) => Math.max(acc, it.y + it.h), 0);
      const newItem: Layout = { i: id, x: 0, y: nextY, w: meta.defaultW, h: meta.defaultH, minW: meta.minW, minH: meta.minH };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="ml-0 md:[margin-left:var(--sidebar-w,280px)] md:transition-[margin-left] md:duration-300 pt-14 min-h-screen">
        ...
        {/* Catalogue de widgets ajoutables */}
        <AnimatePresence>
          {editMode && showAdder && (
            <motion.div
        ...
        {/* La grille */}
        <div className={`px-3 md:px-6 pb-24 ${editMode ? "cockpit-edit" : ""}`}>
          <ResponsiveGrid
            className="layout"
